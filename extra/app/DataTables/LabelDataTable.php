    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function($label) {
                return view('admin.label.datatables-actions', compact('label'))->render();
            })
            ->editColumn('status', function($label) {
                $badgeClass = $label->status === CommonStatusEnum::ACTIVE ? 'success' : 'danger';
                return '<span class="badge bg-'.$badgeClass.' status-badge" style="cursor: pointer;" title="Click to toggle">'.$label->status->label().'</span>';
            })
