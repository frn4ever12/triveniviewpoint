...
class OrderController extends Controller
{
        ...
        try {
            ...
            foreach ($orders as $order) {
                ...
                $invoiceData = [
                    ...
                    'customer_name' => $primaryOrder->customer_name ?? 'Cash Customer',
                    'customer_phone' => $primaryOrder->customer_phone,
    ...
    public function quickCheckout(Request $request, $id)
    {
        ...
        try {
            ...
            $order = Order::with(['invoice'])->where('id', $id)->where('tenant_id', $tenantId)->firstOrFail();
            ...
            $changeAmount = $validated['tender_amount'] - $totalAmount;
    ...
    public function viewBill($orderId)
    {
        ...
        $order = Order::with(['items.menuItem', 'waiter', 'invoice'])
            ->where('id', $orderId)
            ->where('tenant_id', $tenantId)
            ->firstOrFail();

        // Load table only if table_id is valid (not null and not 0)
        if ($order->table_id && $order->table_id > 0) {
            $order->load(['table' => function($query) use ($tenantId) {
        ...
        return view('admin.order.bill', compact('order', 'tenant', 'invoice'));
    }

    public function downloadBillPdf($orderId)
    {
        $tenantId = auth()->user()->tenant_id;
        ...
        $invoiceNumber = $invoice && $invoice->invoice_number ? $invoice->invoice_number : $order->order_no;

        $pdf = \PDF::loadView('admin.order.bill', compact('order', 'tenant', 'invoice'))
            ->setPaper([0, 0, 226.77, 850.39], 'portrait') // 80mm width in points
            ->setOptions([
                'isHtml5ParserEnabled' => true,
                'isRemoteEnabled' => true,
            ]);

        return $pdf->download("bill-{$invoiceNumber}.pdf");
    }
