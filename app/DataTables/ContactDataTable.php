    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function($contact) {
                return view('admin.contact.datatables-actions', compact('contact'))->render();
            })

            ->editColumn('status', function ($contact) {
                $badgeClass = $contact->status === CommonStatusEnum::ACTIVE ? 'success' : 'danger';
                return '<span class="badge bg-' . $badgeClass . ' status-badge" style="cursor: pointer;" title="Click to toggle">' . $contact->status->label() . '</span>';
            })
            ...
            ->editColumn('email', function($contact) {
                return '<strong>'.$contact->email.'</strong>';
            })

            ->editColumn('phone', function($contact) {
                return '<strong>'.$contact->phone.'</strong>';
            })

            ->editColumn('subject', function($contact) {
                return '<strong>'.$contact->subject.'</strong>';
            })
            ->addIndexColumn()
