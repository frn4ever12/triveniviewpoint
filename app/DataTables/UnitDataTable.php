    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function($unit) {
                return view('admin.unit.datatables-actions', compact('unit'))->render();
            })
            ->editColumn('status', function($unit) {
                $badgeClass = $unit->status === CommonStatusEnum::ACTIVE ? 'success' : 'danger';
                return '<span class="badge bg-'.$badgeClass.' status-badge" style="cursor: pointer;" title="Click to toggle">'.$unit->status->label().'</span>';
            })
